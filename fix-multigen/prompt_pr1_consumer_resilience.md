...
This project enforces formatting with `black`. Configuration in `pyproject.toml`:
```
[tool.black]
line-length = 90
target-version = ['py39']
...
**You MUST run `black` on all changed files before committing.** The CI will
reject any PR where `black --check` fails.
...
```python
        ...
        if self.n_total_chunks is None:
            ...
            if (
                ...
                if dfc.n_total_chunks > self.n_total_chunks:
                    ...
                    warnmsg = (
                        ...
                        f"chunk from a newer file generation (had "
                        ...
                        f" from the previous generation and starting"
                        f" fresh."
                    )
            ...
            # in place without changing size. We can't determine which
            # is newer, so stick with the generation we already
            # committed to (skip this chunk) to avoid thrashing when
            # interleaved.
...
```python
    def _reset_for_new_generation(self):
        """Delete the partial file on disk, then reset."""
        if (
...
Replace with:
```python
        # If the file is just in progress (or was reset for a new
...
def test_generation_reset_to_memory(output_dir, logger):
    """
    Verify that DownloadDataFileToMemory resets when a newer
    (larger) generation arrives, and reconstructs the newer
    version correctly.
    """
    from hashlib import sha512
    ...
    # Feed Gen1 chunk 1 — establishes n_total_chunks=2
    result = dl.add_chunk(gen1_chunks[0])
    assert result == DATA_FILE_HANDLING_CONST.FILE_IN_PROGRESS

    # Feed Gen2 chunk 1 — different hash, higher n → RESET
    ...
    for i, chunk in enumerate(gen2_chunks):
        ...
        if i == 0:
            ...
            assert (
                result == DATA_FILE_HANDLING_CONST.FILE_IN_PROGRESS
            )
        else:
            assert (
                result
...
def test_interleaved_generations_to_memory(output_dir, logger):
    """
    Verify that interleaved chunks from two generations are handled
    correctly: newer generation wins, older chunks are skipped.
    """
    from hashlib import sha512
...
def test_equal_chunk_count_no_thrashing(output_dir, logger):
    """
    Verify that two generations with the SAME n_total_chunks but
    different hashes do NOT cause thrashing. The first generation
    committed to wins; the other is skipped entirely.
    """
    from hashlib import sha512
    ...
    # Gen1 and Gen2: same size (3 chunks) but different content
    ...
    assert (
        gen1_chunks[0].n_total_chunks
        == gen2_chunks[0].n_total_chunks
    )
    ...
    # Interleave: Gen1[0], Gen2[0], Gen1[1], Gen2[1], Gen1[2]
    r = dl.add_chunk(gen1_chunks[0])
    assert r == DATA_FILE_HANDLING_CONST.FILE_IN_PROGRESS
...
def test_stale_chunks_skipped_after_reset_to_disk(
    output_dir, logger
):
    """
    Verify that DownloadDataFileToDisk skips stale (older generation)
    chunks after resetting to a newer generation, and reconstructs
    correctly.
    """
    from hashlib import sha512
...
**Format with black FIRST** (the project CI enforces `black --check`):
...
Then run the unit tests (no Kafka broker needed):

```bash
pytest test/test_scripts/test_download_data_file.py -v
```

Then verify black is satisfied (this is what CI checks):
...
All tests must pass and `black --check` must report no changes needed.
