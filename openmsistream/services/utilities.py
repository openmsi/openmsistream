...
def get_os_name():
    ...
    # otherwise I don't know what happened
        ...
        errmsg = (
            f"ERROR: could not determine operating system from platform.system() "
            f'output "{platform.system()}"'
        )
...
def run_cmd_in_subprocess(args, *, logger=None, reraise=False, **other_kwargs):
    """
    run a command in a subprocess and return its result, printing and re-raising
    any exceptions it causes
    """
...
def set_env_var(var_name, var_val):
    ...
    if get_os_name() == "Windows":
        pwrsh_cmd = (
            f'[Environment]::SetEnvironmentVariable("{var_name}","{var_val}",'
...
def remove_env_var(var_name):
    ...
    if get_os_name() == "Windows":
        pwrsh_cmd = (
            f'[Environment]::SetEnvironmentVariable("{var_name}",$null,'
            "[EnvironmentVariableTarget]::Machine)"
        )
        run_cmd_in_subprocess(["powershell.exe", pwrsh_cmd])
    elif get_os_name() == "Linux":
...
def set_env_vars(env_var_names, interactive=True):
    ...
    for env_var_name in env_var_names:
        if (not interactive) and (
            env_var_name in ("KAFKA_PROD_CLUSTER_USERNAME", "KAFKA_PROD_CLUSTER_PASSWORD")
        ):
            continue
        if os.path.expandvars(f"${env_var_name}") == f"${env_var_name}":
            if interactive:
                ...
                raise RuntimeError(
                    f"ERROR: a value for the {env_var_name} environment variable is not set!"
                )
        else:
            if interactive:
                txt = (
                    f"A value for the {env_var_name} is already set, "
...
def test_python_code():
    ...
    must_rerun = set_env_vars(
        ["KAFKA_TEST_CLUSTER_USERNAME", "KAFKA_TEST_CLUSTER_PASSWORD"]
    )
    if must_rerun:
        msg = (
            "New values for environment variables have been set. "
            "Please close this window and rerun InstallService so that their values "
            "get picked up."
        )
        SERVICE_CONST.logger.info(msg)
        sys.exit(0)
    SERVICE_CONST.logger.debug("Testing code to check for errors...")
    test_script_dir_path = (
        pathlib.Path(__file__).parent.parent.parent / "test" / "test_scripts"
    )
