...
class DataFileUploadDirectory(DataFileDirectory,ControlledProcessSingleThread,ProducerGroup,Runnable) :
    ...
    WATCHDOG_OBSERVER_TIMEOUT = 0.1 # amount of time to wait for watchdog to pull events from the queue
    N_STATUS_MESSAGE_FILES = 50 # the max number of files to report status messages for
    ...
    def _on_shutdown(self) :
        ...
        if self.n_partially_done_files>0 :
            msg=(
                'Will finish queueing the following files:\n\t'
    ...
    def __add_active_datafile_for_path(self,filepath) :
        ...
        n_keys_to_remove = len(self.__status_message_files)+1-self.N_STATUS_MESSAGE_FILES
    ...
    def __scrape_dir_for_files(self,retries_left=10) :
        ...
        try :
            for filepath in self.dirpath.rglob('*') :
                ...
                if (filepath not in self.__active_files_by_path) and self.filepath_should_be_uploaded(filepath) :
    ...
    def partially_done_file_paths(self) :
        ...
        """
        return [fp for fp,df in self.__active_files_by_path.items() if df.upload_in_progress]
    @property
