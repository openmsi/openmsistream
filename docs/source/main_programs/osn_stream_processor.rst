...
The user must specify some information on how to connect to the S3 bucket endpoint by adding a new section called ``[osn]`` to the config file used. The parameters that must be specified in this section of the config file are:
...
An example configuration file with an ``[osn]`` section `can be found in the repo here <https://github.com/openmsi/openmsistream/blob/main/openmsistream/my_kafka/config_files/test_osn.config>`_; all the relevant parameters in that example are supplied as environment variables.
...
To run the program in the most common use case, enter the following command and arguments::
    ...
    OSNStreamProcessor [bucket_name] --config [config_file_path] --topic_name [topic_name]
...
* ``[config_file_path]`` is the path to a config file including at least ``[broker]``, ``[consumer]``, and ``[osn]`` sections, and 
...
To see other optional command line arguments, run ``OSNStreamProcessor -h``.
